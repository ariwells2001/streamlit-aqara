...
#        X,Y,test_size= validation_size,stratify=y,random_state = seed)
    ...
    chooseModel = st.selectbox("Choose a Model",("Convolution Neural Network","Deep Neural Network"))
    if chooseModel=="Deep Neural Network":
        X = dataset.iloc[:,0:3]
        y = dataset.iloc[:,3]
        validation_size = 0.10
        seed = 7
        ...
        X_train,X_validation,y_train,y_validation = model_selection.train_test_split(
            X,Y,test_size= validation_size,stratify=y,random_state = seed)
    elif chooseModel=="Convolution Neural Network":
        X = dataset.iloc[:,0:3]
        y = dataset.iloc[:,3]
        validation_size = 0.10
        seed = 7
    ...
    if chooseModel=="Deep Neural Network":
        ...
        no_of_filters = st.slider("No. of Filters", min_value=16,max_value=64,value=32,step=16)
        kernel_size = st.slider("Kernel Size", min_value=2,max_value=10,value=3,step=1)
    ...
    #st.text_area("Model Summary",model.summary(),height=100)
            ...
            model.add(
                tf.keras.layers.Conv1D(no_of_filters,kernel_size,
                                activation='relu',
                                input_shape=(3,1))
                )
            model.add(
    ...
    if ok:
        ...
        if chooseModel=="Deep Neural Network":
            ...
            predictor = [temperature,humidity,air_pressure]
            predictor = np.array(pd.DataFrame(predictor))
            predictor = predictor.reshape(1,3,1)
